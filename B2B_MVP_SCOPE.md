...
**Принцип MVP:** сначала **журнал + заявка + привязка к оборудованию + документы**; склад ЗИП — **фаза 2**.
...
**Не покрывает B2B:** объекты сети, заявки, журналы событий, роли, печать/выгрузка, учёт ЗИП, мульти-тенант организации.
...
| **User** | id, role | admin, dispatcher, technician |

### Сущности фазы 2 (не MVP)

- **SparePart**, **StockMovement**, **MinStock** — склад ЗИП  
...
|---|---------|------|---------------------|
| 1 | Справочник объектов и оборудования | admin | CRUD объектов и агрегатов |
| 2 | Заявка на ремонт | tech, dispatcher | Создание с объекта, статусы: новая → в работе → закрыта |
| 3 | Журнал событий | tech | Запись неисправности/ТО/устранения с привязкой к asset |
| 4 | Документы на оборудовании | tech | Загрузка PDF, просмотр, список по asset |
| 5 | Поиск по документам | tech | Текстовый поиск; опционально Onyx RAG по document set |
| 6 | Выгрузка журнала | admin | PDF/Excel за период по объекту или сети |
...
| 11 | AI-подсказка по симптомам (переиспользовать Chat + persona на asset) |
...
- B2C сценарий Atlant в том же onboarding  
...
| Export | Выбор периода и объекта → share PDF |
...
|-----------|-------------------|
| `ChatApi`, `HttpChatRepository`, стриминг | AI на карточке asset |
...
| Wasm/Android/iOS targets | Те же платформы; приоритет **Android + Web** для техника |
...
| **2** | +6 нед | ЗИП, минимальные остатки, списание по заявке |
| **3** | +8 нед | PPR, чек-листы, интеграция 1С (если custdev подтвердит) |
...
2. Техник создаёт заявку и запись журнала с телефона &lt; 2 мин.  
...
| Scope creep (ЗИП, PPR) | Жёсткий won't-have; фаза 2 |
