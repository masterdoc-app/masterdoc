# Masterdoc — маркетинг и продуктовая документация

Репозиторий лендинга **masterdoc.pro** и стратегических материалов B2B.

## Продуктовые документы (B2B эксплуатация)

| Документ | Назначение |
|----------|------------|
| [STRATEGY.md](STRATEGY.md) | ICP, one-liner, GTM, метрики |
| [TOIR_AI_SYSTEM_DESIGN.md](TOIR_AI_SYSTEM_DESIGN.md) | **Проект системы ТОиР для малого бизнеса: конкуренты, AI-агенты, стейты, рабочие места** |
| [DIGITIZATION_SEARCH_INTENT.md](DIGITIZATION_SEARCH_INTENT.md) | **Что ищут те, кто хочет оцифровать журналы и ИД** |
| [SEO_KEYWORDS.md](SEO_KEYWORDS.md) | Матрица Wordstat для лендинга и рекламы |
| [WORDSTAT_TOIR_AUTOMATION_LEADS.md](../masterdoc-toir/WORDSTAT_TOIR_AUTOMATION_LEADS.md) | **ТОиР: лиды на автоматизацию** (без 1С и журналов), свежий Wordstat |
| [COMPETITOR_BATTLECARDS.md](COMPETITOR_BATTLECARDS.md) | Excel, 1С:ТОиР, Hubex, Okdesk |
| [CUSTDEV_GUIDE.md](CUSTDEV_GUIDE.md) | Скрипт 5 интервью (ритейл / холод) |
| [CUSTDEV_INTERVIEWS.md](CUSTDEV_INTERVIEWS.md) | Журнал интервью |
| [B2B_MVP_SCOPE.md](B2B_MVP_SCOPE.md) | MVP vs текущий B2C app |

## Прочее

| Документ | Назначение |
|----------|------------|
| [PRODUCT_NOTE.md](PRODUCT_NOTE.md) | Референс Pairio |
| [KNOWLEDGE_BASE.md](KNOWLEDGE_BASE.md) | База знаний B2C (Atlant FAQ) |

## Код приложения

- [masterdocapp](https://github.com/masterdoc-app/masterdocapp) — KMP клиент  
- [backend](../backend/) — API и Onyx  

## Лендинг

Статика в [landing/](landing/).
