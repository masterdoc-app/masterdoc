...
**Дата:** 2026-07-09 · актуализация 2026-07-18
...
flowchart TB
  ...
  User -->|диспетчер: доки + оборудование<br/>(ИЭ / руководство)| Technologist
  Technologist -->|карточки автоматически:<br/>draft Asset + регламент + плановая| Asset
...
Смысл схемы: **актив** — центр системы. **Admin** настраивает систему (типы оборудования, пользователи, доступы); **диспетчер** ведёт операционные справочники (объекты, активы, документы). От актива идут QR, заявки, журнал и регламент. **Технолог** (UI у диспетчера): диспетчер загружает **доки и оборудование** (ИЭ / руководство по ремонту) — **карточки создаются автоматически** (draft Asset + график обслуживания: регламент, чек-лист, плановая заявка). Диспетчер при необходимости правит и подтверждает. Шильдик/QR в поле — только у **инженера**, чтобы найти уже заведённый актив. Если в инструкции указаны расходники или запчасти, Технолог добавляет это текстовой пометкой в чек-лист, но складского учёта в первых релизах нет. Заявка — основной рабочий объект: через неё инженер выполняет работу, пополняет журнал и создаёт историю по оборудованию. AI-агенты не заменяют учёт: они создают черновики, ищут по документам и структурируют текст. Отчёты — отдельный read-only модуль поверх базы и агрегатов, без LLM.
...
| **Limble** (Asset Snap, Winter Release 2026) | MCP-слой; AI Resource Planning; карточка актива из фото шильдика | Шильдик у нас **не** источник карточки: в поле ходит инженер (идентификация уже заведённого актива). **Технолог** у диспетчера: загрузка доков и оборудования → карточки автоматически |
...
1. **Категория уже назвала паттерны, их можно не изобретать:** текст/фото → заявка; RAG-чат по мануалам у станка (все); мануал → процедура/чек-лист и регламент ТО (MaintainX, UpKeep) — у нас это **Технолог**: диспетчер загружает доки и оборудование, карточки создаются автоматически. Паттерн «фото шильдика → актив» (Limble) **не берём**: шильдик/QR — только идентификация в поле у инженера. Складские сценарии фиксируем как backlog, но не включаем в начальный скоуп.
...
|-------|--------|-------------------|--------------|---------------------|--------|------|
| **Технолог** | Диспетчер загружает **доки и оборудование** (ИЭ / руководство по ремонту) → **карточки создаются автоматически**: draft Asset + график ТО (регламент, чек-лист, плановая заявка) | Каталог типов оборудования; формат полей Asset; отраслевые шаблоны регламентов; только загруженные документы пакета | Извлекай из доки, не выдумывай; нет данных — пусто + confidence; каждый пункт чек-листа — со ссылкой на страницу; расходники/ЗИП — только текстовая пометка в чек-листе; диспетчер при необходимости правит и подтверждает. **Не** использует фото шильдика | createDraftAsset, createDraftPlan, createDraftChecklist, createDraftWorkOrder(type: preventive) | Тяжёлая LLM (редкие вызовы) + при необходимости VLM по страницам PDF | MVP |
...
- **Один агент — одна метрика.** Технолог: % полей Asset и пунктов графика/чек-листа, принятых диспетчером без правки; Наставник: доля ответов с корректной цитатой; Приёмщик: % draft-заявок, подтверждённых без редактирования. Метрики видны нам (eval) и клиенту (доверие).
- **Знания раздаются по минимуму.** Наставник не видит чужие активы, Приёмщик видит только активы/заявки в своём контексте, Технолог видит только документы загруженного пакета (и каталог типов). Это же ответ на 152-ФЗ: контур каждого агента понятен и документируем.
...
| **MaintenancePlan** | Регламент ТО актива (в малом бизнесе — простая периодичность, не полноценный ППР); создаётся как черновик **Технологом** по доке (ИЭ / руководство по ремонту) | id, assetId, title, интервал (дни/моточасы), checklistId, nextDueAt, isActive, source (ai_generated / manual / template) |
...
- **dispatcher (операции):** `Site`, `Asset` — объекты сети и единицы оборудования; загрузка **доков и оборудования**; подтверждение/правка пакета от Технолога (карточки Asset + график ТО).
- Технолог при загрузке **автоматически создаёт карточки** (Asset + график ТО); активирует **диспетчер**. Шильдик/QR — не вход Технолога.
...
flowchart TB
  ...
  subgraph Agents["AI-агенты"]
    ...
    Technologist["Технолог<br/>доки + оборудование →<br/>карточки автоматически"]
  ...
  PreventiveDoc -->|диспетчер загрузил ИЭ / руководство| Technologist
  Technologist -->|draft Asset + preventive WO<br/>+ checklist + MaintenancePlan| Draft
...
- `preventive` / плановая заявка: создаётся **Технологом** по доке (ИЭ / руководство по ремонту) вместе с draft Asset и регламентом, либо календарём активного регламента; дальше живёт как обычная WorkOrder. **Утверждает только диспетчер**. Шильдик не участвует.
...
Ключевое правило: **диспетчер загружает доки и оборудование (ИЭ / руководство по ремонту) — агент Технолог автоматически создаёт связанный пакет карточек** (UI у диспетчера):

1. `Asset` — карточка единицы оборудования.
2. `MaintenancePlan` — что и как часто делать (график обслуживания).
3. `Checklist` — шаги выполнения.
4. `WorkOrder` типа `preventive` в статусе `draft` — первая плановая заявка на ремонт/ТО.

Вход — **дока и оборудование**, не фото шильдика (в поле ходит только инженер; шильдик/QR — идентификация уже заведённого актива). Если в документации указаны расходники или запчасти, Технолог добавляет это как текстовую пометку в чек-лист («проверить наличие/подготовить ...»), но **не создаёт складскую заявку**. **Диспетчер** при необходимости правит и подтверждает пакет. После подтверждения: Asset и регламент становятся `active`, плановая заявка переходит `draft → new`.
...
`draft` (создан диспетчером вручную или подготовлен Технологом, не подтверждён) → `active` (подтвердил диспетчер) → `inactive` (законсервирован) → `retired` (списан). Плюс вычисляемый операционный флаг `down` (есть открытая заявка с признаком «оборудование остановлено») — для дашборда диспетчера.
...
Ключевые отличия от enterprise-ТОиР: **planner/инженер по надёжности отсутствует** — его функцию (карточка актива + регламенты и чек-листы из доки) выполняет агент **Технолог** + **подтверждение диспетчера**; **нет отдельного разделения engineer/technician** — есть одна роль `engineer`; **contractor не входит в первые релизы**; **admin отвечает только за конфигурацию системы**, операционные справочники (объекты, активы, документы) ведёт **диспетчер**; **склада и кладовщика в первых релизах нет**.
...
Экраны: Доска заявок по сети → **Входящие от ИИ** (draft от Приёмщика, пакеты Технолога: Asset + график ТО) → **Объекты и оборудование** (Site, Asset, подтверждение пакета Технолога) → **Документы** (загрузка доков и оборудования → Технолог, карточки автоматически) → Календарь ТО → Журналы (просмотр). QR/шильдик в поле — экран **инженера**, не диспетчера.
...
flowchart TB
  ...
  subgraph Агенты["AI-агенты (§4.2) — у каждого свой промпт, знания и tools"]
    ...
    A_TECH["Технолог<br/>(доки + оборудование →<br/>карточки автоматически)"]
  end
  ...
  DISP -- "доки + оборудование" --> A_TECH
  A_TECH -- "карточки автоматически<br/>(Asset + график ТО)" --> DISP
  TECH -- "QR / шильдик<br/>(найти уже заведённый актив)" --> SYS
...
3. **Справочный / плановый** (диспетчер → Технолог → учёт): **диспетчер** загружает **доки и оборудование**; **Технолог** **автоматически создаёт карточки** (Asset + график ТО); диспетчер при необходимости правит и подтверждает. Admin только настраивает типы (`EquipmentCategory`). Шильдик/QR — не вход Технолога: в поле ходит **инженер**, чтобы найти уже заведённый актив.
...
sequenceDiagram
  ...
  Disp->>S: загрузил ИЭ / руководство по ремонту
  S->>T: событие DocumentAttached (или пакет «новая единица»)
  T->>S: draft Asset + draft MaintenancePlan + draft Checklist
  T->>W: draft плановая заявка preventive
  W->>S: sourceDocumentId + sourcePage
  Disp->>S: подтверждает пакет черновиков
  S->>S: Asset draft → active; WorkOrder draft → new
```

Критично: **Технолог** при загрузке **доков и оборудования** **автоматически создаёт карточки** (Asset + график ТО: регламент + чек-лист + плановая заявка); UI у **диспетчера**. Шильдик не используется. Если в документации указаны расходники или запчасти, это остаётся текстовой пометкой в чек-листе до появления складского контура в backlog.
...
| **Auth** | **Zitadel self-host (РФ)** — OIDC; вход только по **email + пароль**, регистрация **только по invite** (§8.1); канон — [`masterdoc-zitadel`](https://github.com/masterdoc-app/masterdoc-zitadel) |
...
**Канон, deploy, Terraform platform, verify:** приватный репозиторий [`masterdoc-app/masterdoc-zitadel`](https://github.com/masterdoc-app/masterdoc-zitadel) (`docs/AUTHORIZATION.md`, `docs/RUNBOOK.md`). Ниже — краткий продуктный обзор.
...
| **dispatcher** | CRUD `Site`, `Asset`; загружает ИЭ / руководство по ремонту; подтверждает пакет Технолога (`draft Asset` + график ТО) |
| engineer | Читает карточку; в поле сканирует QR / шильдик, чтобы найти уже заведённый актив |
| ai-gateway (Технолог) | `createDraftAsset` (+ график ТО пишет в dashboard, см. ниже) |

**Не делает:** заявки, журнал, файлы как бинарное хранилище (это document-service).

**Пример:** диспетчер загрузил ИЭ «Saeco» → Технолог → draft Asset «Saeco №1» + график ТО → диспетчер подтвердил → инженер клеит QR и сканирует его в поле.
...
| dispatcher | Подтверждает пакет черновиков от Технолога (Asset + план + checklist + preventive WO) |
| ai-gateway (Приёмщик) | `createDraftWorkOrder(type: corrective)` |
| ai-gateway (Писарь) | `draftCloseout` + `draftJournalEntry` |
| ai-gateway (Технолог) | `createDraftAsset` + `createDraftPlan` + `createDraftChecklist` + `createDraftWorkOrder(type: preventive)` |
...
2. **Плановая:** диспетчер загрузил ИЭ / руководство по ремонту → Технолог создал draft-пакет (Asset + график ТО) → диспетчер подтвердил → scheduler создаёт следующие `preventive` → инженер выполняет.
...
| ai-gateway (Технолог) | Читает метаданные и содержимое документа для анализа |
...
**Пример:** диспетчер загрузил `ie_vitrina.pdf` (ИЭ / руководство по ремонту) → search индексирует → Технолог создаёт draft-пакет (Asset + график ТО) на подтверждение диспетчеру.
...
|-------|------|------------------|----------|
| Технолог | доки + оборудование (UI диспетчера) | catalog: Asset; dashboard: plan, checklist, preventive WO (карточки автоматически) | document, catalog |
...
| Компонент | Задача |
|-----------|--------|
| **Zitadel self-host (РФ)** | Логин email+пароль, invite, роли в JWT |
...
| **catalog-service** | `Site`, `EquipmentCategory`, `Asset` | `GET/POST /sites`, `GET/POST /assets`, `GET/POST /equipment-categories`, `POST /assets/from-documents` → draft (Технолог) | Отдаёт asset/site по id для dashboard/ai |
...
| **ai-gateway** | `agent_run_log`, конфиг агентов | `POST /ai/technologist`, `POST /ai/intake`, `POST /ai/mentor`, `POST /ai/scribe` | Детерминированная маршрутизация §4.2; tools → draft в catalog/dashboard |
...
|---------|-----------|-----------|-------|
| `asset.draft.created` | catalog | notification | Диспетчеру: подтвердить карточку от Технолога |
| `document.attached` | document | search, ai-gateway | Индексация + Технолог → draft Asset + график ТО |
...
**Загрузка доков и оборудования → Технолог (карточки автоматически):**
...
sequenceDiagram
  ...
  Disp->>DOC: POST /documents (ИЭ / руководство по ремонту)
  DOC->>BUS: document.attached
  BUS->>SRCH: индексация
  BUS->>AI: Технолог
  AI->>CAT: createDraftAsset
...
| **2** | dashboard-service: + MaintenancePlan, Checklist, scheduler, календарь | Технолог (полный пакет: Asset + график ТО) |
...
Новые endpoint'ы поверх черновика API из B2B_MVP_SCOPE: `POST /assets/from-documents` (дока → draft Asset + график ТО через Технолога), `POST /documents` (загрузка ИЭ / руководства по ремонту → событие для Технолога), `POST /documents/{id}/analyze-maintenance` (ручной перезапуск анализа), `GET/POST /maintenance-plans`, `POST /work-orders/text` (текст/фото → draft), `POST /work-orders/{id}/closeout/text`.
...
|------|------------|-----------|
| **1 MVP** | Сущности и экраны по B2B_MVP_SCOPE: объекты, каталог типов, единицы оборудования, заявки, журнал, документы, выгрузка | **Технолог** (загрузка доков и оборудования → карточки автоматически) |
| **1.1** | Фото, push, QR-карточка актива (инженер в поле), опциональные заявки от авторизованных пользователей объекта | Приёмщик (текст/фото → draft-заявка), Наставник (чат по докам на активе), Писарь (текстовое закрытие) |
| **2** | MaintenancePlan + Checklist, календарь ТО / scheduler | Углубление пакета Технолога + календарь поверх подтверждённых регламентов |
...
| Риск | Митигация |
|------|-----------|
| Точность извлечения (неполная/сканированная дока, неполное описание дефекта) | Human-in-the-loop везде; метрика % принятых без правки; fallback на ручной ввод в том же экране |
