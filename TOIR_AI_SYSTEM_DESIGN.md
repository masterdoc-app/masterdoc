...
flowchart TB
  subgraph Org["Организация / сеть"]
    ...
    User["Пользователи<br/>руководитель · диспетчер · инженер · кладовщик"]
  end
...
stateDiagram-v2
  ...
  InProgress --> OnHold: ждём запчасть
  OnHold --> InProgress
...
stateDiagram-v2
  ...
  [*] --> Needed: инженер в заявке отметил нужна запчасть
...
| **Кладовщик / снабженец** (storekeeper) | Web + мобильный сканер | Роль обязательна с фазы 2, но **совмещаемая**: в микро- и малом сегменте её держит руководитель или сам инженер |

Ключевые отличия от enterprise-ТОиР: **planner/инженер по надёжности отсутствует** — его функцию (составление регламентов и чек-листов) выполняет агент Технолог + подтверждение руководителя; **нет отдельного разделения engineer/technician** — есть одна роль `engineer`, которая и создаёт внеочередные заявки, и выполняет работы; **contractor не входит в первые релизы** — внешний подрядчик в будущем будет тем же инженером, но не в штате; **каталог оборудования и единицы оборудования ведёт админ**, а инженер работает по уже заведённым активам; **выделенного кладовщика нет** — складские операции размазаны по рабочим местам инженера (списание в заявке) и руководителя (уведомления о min-остатках, приход по накладной).
...
| Где | Функция |
|-----|---------|
| Заявка инженера (mobile) | «Отметить использованные запчасти» — поиск/скан → qty → StockMovement `issue` при закрытии заявки (паттерн MaintainX); «нужна запчасть» → PartRequest, заявка сама уходит в `on_hold` |
...
4. **Складской** (Технолог/инженер → кладовщик ↔ Снабженец → учёт): PartRequest всегда привязан к WorkOrder; для плановых работ заявки на ЗИП создаёт Технолог из документации, для внеплановых — инженер из карточки заявки. Движения ЗИП всегда привязаны к заявке.
...
| **Backlog** | Массовый AI-онбординг: пакетные фото шильдиков, оцифровка бумажных журналов, импорт Excel промптами; голосовой ввод и STT; внешний инженер/contractor не в штате | Возвращаем по сигналам custdev |
