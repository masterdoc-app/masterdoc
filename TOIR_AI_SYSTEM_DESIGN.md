...
flowchart TB
  subgraph Org["Организация / сеть"]
    ...
    User["Пользователи<br/>руководитель · диспетчер · исполнитель · кладовщик"]
  end
  ...
  Asset -->|исполнитель создаёт заявку| Intake
...
| Малый+ (15–50 объектов) | Региональная сеть, франшиза | Руководитель эксплуатации, диспетчер (часто совмещённый), 5–20 исполнителей, подрядчики |
...
| **Писарь** | Текстовый отчёт исполнителя → структурированное закрытие + запись журнала | Текущая заявка (актив, чек-лист); каталог ЗИП для распознавания названий запчастей | Заполни «дефект/причина/действия/запчасти» только из введённого текста | draftCloseout, draftJournalEntry, matchParts | Быстрая дешёвая LLM | 1.1 |
...
- Исполнитель использует существующий Asset: создаёт внеочередные заявки, смотрит документы, закрывает работы, но не ведёт каталог и не создаёт единицы оборудования.
...
stateDiagram-v2
  ...
  Assigned --> InProgress: исполнитель взял в работу
...
- **Исполнитель** — базовый и обязательный сценарий: создаёт заявку из карточки актива или из своего рабочего места после осмотра/обхода/сообщения с объекта.
...
Приёмщик может подготовить `draft WorkOrder` только для разрешённого источника. Если фича `userRequestsEnabled` выключена, вход от пользователя объекта не превращается в заявку; её должен завести исполнитель.
...
stateDiagram-v2
  ...
  [*] --> Needed: исполнитель в заявке отметил нужна запчасть
...
| **Исполнитель** (executor: техник / мастер / инженер) | Мобильное приложение (Android приоритет) | Обязательна |
...
| **Пользователь объекта** (requester: продавец, повар, оператор) | Авторизованный web/mobile/бот; создание внеочередных заявок только при включённой фиче `userRequestsEnabled` | Опциональна — можно не подключать, тогда внеочередные заявки заводит исполнитель |
| **Кладовщик / снабженец** (storekeeper) | Web + мобильный сканер | Роль обязательна с фазы 2, но **совмещаемая**: в микро- и малом сегменте её держит руководитель или сам исполнитель |

Ключевые отличия от enterprise-ТОиР: **planner/инженер по надёжности отсутствует** — его функцию (составление регламентов и чек-листов) выполняет агент Технолог + подтверждение руководителя; **нет отдельного разделения engineer/technician** — есть одна роль `executor`, которая и создаёт внеочередные заявки, и выполняет работы; **каталог оборудования и единицы оборудования ведёт админ**, а исполнитель работает по уже заведённым активам; **выделенного кладовщика нет** — складские операции размазаны по рабочим местам исполнителя (списание в заявке) и руководителя (уведомления о min-остатках, приход по накладной).

### 7.2 Рабочее место исполнителя (мобильное)
...
| Где | Функция |
|-----|---------|
| Заявка исполнителя (mobile) | «Отметить использованные запчасти» — поиск/скан → qty → StockMovement `issue` при закрытии заявки (паттерн MaintainX); «нужна запчасть» → PartRequest, заявка сама уходит в `on_hold` |
...
flowchart TB
  subgraph Объект["Объект (магазин, кафе, цех)"]
    ...
    TECH["Исполнитель<br/>(техник / мастер / инженер)"]
...
1. **Заявочный** (исполнитель или авторизованный пользователь при включённой фиче → Приёмщик → диспетчер → исполнитель/подрядчик → учёт): внеочередную заявку нельзя создать анонимно. QR только определяет актив и открывает разрешённое действие; Приёмщик структурирует, ищет дубли и проверяет feature flag, диспетчер подтверждает и назначает.
2. **Исполнительский** (исполнитель ↔ Наставник/Писарь ↔ учёт): у станка исполнителя обслуживают два разных агента — read-only Наставник отвечает по документации, Писарь оформляет сказанное в закрытие и журнал; журнал пополняется как побочный эффект работы.
3. **Справочный / плановый** (админ → Паспортист/Технолог → учёт): каталог оборудования и единицы оборудования создаёт админ; Паспортист только помогает заполнить карточку по шильдику. При добавлении документации к активу Технолог создаёт связанный пакет черновиков — регламент, плановую заявку и заявки ЗИП. Админ подтверждает пакет, а не создаёт плановые работы вручную.
4. **Складской** (Технолог/исполнитель → кладовщик ↔ Снабженец → учёт): PartRequest всегда привязан к WorkOrder; для плановых работ заявки на ЗИП создаёт Технолог из документации, для внеплановых — исполнитель из карточки заявки. Движения ЗИП всегда привязаны к заявке.
...
| Слой | Решение |
|------|---------|
| Клиенты | KMP (Decompose, MVIKotlin) — Android для исполнителя, Web для диспетчера (как в B2B_MVP_SCOPE); авторизованный web/mobile/бот для пользователей объекта только при включённой фиче `userRequestsEnabled` |
...
| **1.1** | Фото, push, QR-карточка актива, опциональные заявки от авторизованных пользователей объекта, подрядчик по ссылке | Приёмщик (текст/фото → draft-заявка от исполнителя или авторизованного пользователя при включённой фиче), Наставник (чат по докам на активе), Писарь (текстовое закрытие) |
...
> «1С:ТОиР внедряют квартал. HubEx подключает ИИ за две недели и отдельные деньги. У нас исполнитель заводит внеочередную заявку текстом и фото прямо с карточки оборудования, спрашивает инструкцию у агрегата, а журнал под проверку собирается сам — из работы, а не вместо неё.»
