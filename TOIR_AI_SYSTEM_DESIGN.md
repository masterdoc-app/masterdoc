...
Система ТОиР для малого бизнеса (1–50 объектов, 1–20 техников), в которой **ИИ убирает главный барьер входа — заполнение системы данными**. Классический CMMS умирает у малого бизнеса на этапе «занесите 200 единиц оборудования, настройте регламенты, обучите людей». Мы проектируем систему, где онбординг делается **фотографией шильдика и голосом**, а регламенты ТО и чек-листы **генерируются ИИ из паспортов и инструкций**. Учёт и журналы остаются ядром ценности (см. STRATEGY: «AI — усилитель, не замена учёта»), но именно ИИ сокращает время до первой пользы с недель до часа.
...
Вывод: **у малого бизнеса нет ни админа, ни интегратора, ни времени**. Единственный жизнеспособный путь внедрения — система сама строит справочники из того, что у клиента уже есть: шильдики, тетради, PDF, WhatsApp-переписки.
...
| 2 | **Регламент из документа** | PDF паспорта/ИЭ (или наш типовой шаблон по категории) | План ТО (периодичность) + чек-лист работ, привязанные к активу | MaintainX «manual → procedure», UpKeep Smart Checklist |
| 3 | **Заявка голосом/фото** | Голосовое сообщение или фото дефекта (в т.ч. из Telegram/WhatsApp) | Оформленная заявка: актив угадан по QR/гео/названию, симптом структурирован | UpKeep Voice Fill, Pairio, Okdesk-автозаполнение |
| 4 | **Оцифровка бумажного журнала** | Фото страниц тетради/распечатки | Записи журнала задним числом с датами и активами | Нет у конкурентов — наш дифференциатор под compliance-спрос из [DIGITIZATION_SEARCH_INTENT.md](DIGITIZATION_SEARCH_INTENT.md) |
| 5 | **Запчасть по фото / накладной** (фаза 2) | Фото запчасти, коробки или накладной поставщика | Карточка SparePart (или пачка карточек из накладной) + приход на склад | UpKeep Photo-to-Part; распознавание накладной — нет у конкурентов |

Сценарий 4 уникально ложится на наш рынок: Wordstat показывает, что клиент живёт в «журнал → образец → скачать»; фото его текущей тетради — идеальный мост из бумаги в систему.

**Отложено (backlog): импорт «грязного» Excel промптами** (паттерн UpKeep Data Transformations). Сознательно выведен из скоупа для упрощения MVP: у микро- и малого сегмента таблицы либо нет вовсе, либо она короткая — фото-поток Паспортиста покрывает вход за те же 15–30 минут, а Excel-импорт тянет за собой отдельный контур маппинга колонок, правок промптами и batch-review UI. Возвращаем в скоуп, если custdev покажет, что у сегмента 15–50 объектов Excel-реестры длинные и живые.
...
| **Технолог** | PDF паспорта/ИЭ → план ТО + чек-лист (§5.2 #2) | Только документы данного актива + отраслевые шаблоны регламентов | Каждый пункт чек-листа — со ссылкой на страницу источника; нет источника — пометь как «типовой» | createDraftPlan, createDraftChecklist | Тяжёлая LLM (редкие вызовы) | Фаза 2 |
| **Архивариус** | Фото бумажного журнала → записи задним числом (§5.2 #4) | Формы типовых журналов (ТО, неисправности); список активов и людей организации для сопоставления | Распознай строки, сопоставь активы, даты не выдумывай | createDraftJournalEntries | VLM тяжёлая | Фаза 2 |
| **Приёмщик** | Голос/фото/текст из QR-формы или бота → draft-заявка (§5.2 #3) | Список активов объекта (по QR/гео); категории дефектов; открытые заявки (антидубль) | Структурируй симптом, определи актив, проверь дубль; не диагностируй | createDraftWorkOrder, findAsset, findDuplicates | Быстрая дешёвая LLM | 1.1 |
...
| **Снабженец** | Фото запчасти/накладной → карточки и приход (§5.2 #5); лист «заканчивается» | Каталог SparePart организации; словарь артикулов поставщиков | Извлеки позиции накладной, сопоставь с каталогом, новое — как черновик | createDraftParts, createDraftReceipt, getStockBalances | VLM среднего класса | Фаза 2 |
...
| **ImportSession** | Единица AI-онбординга: пачка фото/файл/диктовка → черновики → подтверждение | id, type (nameplate / document / journal_scan / voice / part_photo / invoice), status, inputFiles[], draftEntities[], confirmedBy, stats |
...
stateDiagram-v2
  ...
  NeedsReview --> PartiallyConfirmed: часть черновиков принята
  PartiallyConfirmed --> NeedsReview: правки остальных
...
flowchart TB
  ...
  subgraph Агенты["AI-агенты (§5.4) — у каждого свой промпт, знания и tools"]
    ...
    A_ONB["Паспортист · Технолог ·<br/>Архивариус<br/>(онбординг, ImportSession)"]
  ...
  ADMIN -- "фото шильдиков, PDF,<br/>сканы тетрадей" --> A_ONB
...
3. **Онбординговый** (руководитель → Паспортист/Технолог/Архивариус → диспетчер): агенты онбординга работают только внутри ImportSession и выдают черновики на подтверждение.
...
| 2 | Идёт по объекту и фотографирует шильдики (поточный режим: фото → следующее) | ImportSession с Паспортистом: пачка фото → черновики карточек → подтверждение списком | 15–30 мин на 20–50 активов |
...
**Целевая метрика активации:** организация с ≥10 активами, ≥1 регламентом и ≥1 закрытой заявкой в первые 7 дней. Дополнительные метрики: % черновиков ImportSession, принятых без правки (>80%), точность извлечения шильдика (>90% по полям производитель/модель).
...
|------|------------|--------------|
| **1 MVP** | Сущности и экраны по B2B_MVP_SCOPE + ImportSession | Сценарий §5.2 #1 (шильдик) — это и есть лид-магнит вместо «пустой системы» |
| **1.1** | Фото, push, AI-чат на активе | #3 заявка голосом; QR-заявитель |
| **2** | MaintenancePlan + Checklist, календарь ТО; **склад ЗИП** (§6, §7.6, §8.5): SparePart, StockMovement, PartRequest, min-остатки | #2 регламент из документа; #4 оцифровка бумажного журнала; #5 Photo-to-Part и приход по фото накладной |
...
| **Backlog** | Импорт «грязного» Excel промптами (агент Импортёр) — см. пометку в §5.2 | Возвращаем, если custdev подтвердит живые Excel-реестры у сегмента 15–50 объектов |
...
> «1С:ТОиР внедряют квартал. HubEx подключает ИИ за две недели и отдельные деньги. У нас: пройдите по объекту и сфотографируйте шильдики — через час у вас работающий учёт, журналы под проверку и план ТО, собранный ИИ из ваших же инструкций.»
