...
| **Приёмщик** | Текст/фото из формы инженера или авторизованного пользователя → draft-заявка внеочередного ремонта | Список активов объекта; категории дефектов; открытые заявки (антидубль); флаг `userRequestsEnabled` | Структурируй симптом, определи актив, проверь дубль; не диагностируй; если пользовательские заявки выключены — не создавай draft от пользователя | createDraftWorkOrder, findAsset, findDuplicates, checkFeatureFlag | Быстрая дешёвая LLM | 1.1 |
...
- Инженер/техник использует существующий Asset: создаёт внеочередные заявки, смотрит документы, закрывает работы, но не ведёт каталог и не создаёт единицы оборудования.
...
- **Инженер / техник** — базовый и обязательный сценарий: создаёт заявку из карточки актива или из своего рабочего места после осмотра/обхода/сообщения с объекта.
...
- **Анонимный заявитель без аккаунта** — не входит в базовый скоуп. QR без авторизации открывает справочную карточку/контакт инженера, но не создаёт WorkOrder.

Приёмщик может подготовить `draft WorkOrder` только для разрешённого источника. Если фича `userRequestsEnabled` выключена, вход от пользователя объекта не превращается в заявку; её должен завести инженер/техник.
...
| **Пользователь объекта** (requester: продавец, повар, оператор) | Авторизованный web/mobile/бот; создание внеочередных заявок только при включённой фиче `userRequestsEnabled` | Опциональна — можно не подключать, тогда внеочередные заявки заводит инженер/техник |
...
Ключевые отличия от enterprise-ТОиР: **planner/инженер по надёжности отсутствует** — его функцию (составление регламентов и чек-листов) выполняет агент Технолог + подтверждение руководителя; **каталог оборудования и единицы оборудования ведёт админ**, а инженер/техник работает по уже заведённым активам; **выделенного кладовщика нет** — складские операции размазаны по рабочим местам техника (списание в заявке) и руководителя (уведомления о min-остатках, приход по накладной).
...
1. **Заявочный** (инженер/техник или авторизованный пользователь при включённой фиче → Приёмщик → диспетчер → техник/подрядчик → учёт): внеочередную заявку нельзя создать анонимно. QR только определяет актив и открывает разрешённое действие; Приёмщик структурирует, ищет дубли и проверяет feature flag, диспетчер подтверждает и назначает.
...
| Слой | Решение |
|------|---------|
| Клиенты | KMP (Decompose, MVIKotlin) — Android для техника/инженера, Web для диспетчера (как в B2B_MVP_SCOPE); авторизованный web/mobile/бот для пользователей объекта только при включённой фиче `userRequestsEnabled` |
...
| **1.1** | Фото, push, QR-карточка актива, опциональные заявки от авторизованных пользователей объекта, подрядчик по ссылке | Приёмщик (текст/фото → draft-заявка от инженера или авторизованного пользователя при включённой фиче), Наставник (чат по докам на активе), Писарь (текстовое закрытие) |
...
> «1С:ТОиР внедряют квартал. HubEx подключает ИИ за две недели и отдельные деньги. У нас инженер заводит внеочередную заявку текстом и фото прямо с карточки оборудования, техник спрашивает инструкцию у агрегата, а журнал под проверку собирается сам — из работы, а не вместо неё.»
