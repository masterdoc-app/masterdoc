...
flowchart TB
  subgraph Org["Организация / сеть"]
    ...
    User["Пользователи<br/>руководитель · диспетчер · инженер"]
  end
  ...
  Journal --> Analyst
  Request --> Analyst
  Analyst -->|дайджест: просрочки, повторы| User
```
...
| **Аналитик** | Недельный дайджест руководителю: просрочки, повторные отказы, активы-«пожиратели» времени (лайт-версия Fiix Foresight / ТОиР Аналитик) | Агрегаты учёта: заявки, простои, повторы; пороговые правила (повторный отказ, рост просрочек) | Только факты из данных с цифрами и ссылками на заявки; без советов «купите новое» | queryAggregates (read-only) | Тяжёлая LLM (1 вызов/нед.) | Фаза 3 |
...
### 7.5 Схема взаимодействия ролей и агентов
...
4. **Управленческий** (учёт → Аналитик → руководитель): руководитель ничего не вводит — смотрит дашборд, подтверждает регламенты, читает дайджест, выгружает журналы к проверке.
...
| **3** | Дайджест-аналитика руководителю | Аналитик |
