...
| **Limble** (Asset Snap, Winter Release 2026) | MCP-слой (доступ LLM-клиентов к данным CMMS read-only); AI Resource Planning (балансировка загрузки техников); карточка актива из фото шильдика | Паспортист как помощник админа при создании новой единицы оборудования с нуля |
...
- `preventive` / плановая заявка: создаётся Технологом при добавлении документации к активу или календарём активного регламента; дальше живёт как обычная WorkOrder. **Утверждает только диспетчер** — не admin (admin может загрузить документацию, но не подтверждает заявку на ТО).
...
Экраны диспетчера: Доска заявок по сети (фильтр: объект, статус, просрочка — из B2B_MVP_SCOPE must-have №7) → **Входящие от ИИ** (draft-заявки от Приёмщика и **пакеты Технолога** — draft-регламент + draft-плановая заявка + checklist; **подтверждает диспетчер**, не admin) → Календарь ТО (ближайшие плановые).

Экраны admin (если роль совмещена с диспетчером — те же вкладки, но права разные): Каталог оборудования → Единицы оборудования (подтверждение draft от Паспортиста, печать QR) → загрузка документов к активу → Журналы + выгрузка PDF/Excel → Справочники (объекты, люди).
...
3. **Справочный / плановый** (админ → Паспортист/Технолог → диспетчер → учёт): каталог оборудования и единицы оборудования создаёт админ; Паспортист только помогает заполнить карточку по шильдику. При добавлении документации к активу Технолог создаёт связанный пакет черновиков — регламент и плановую заявку. **Диспетчер** подтверждает пакет (admin загружает документы, но не утверждает заявку на ТО).
...
sequenceDiagram
  ...
  Admin->>S: добавил паспорт / ИЭ к активу
...
| Роль | Действие |
|------|----------|
| admin | Назначает инженеру доступ к объектам «Магазин №3» и «Склад» |
| admin | Включает/выключает заявки от requester |
...
**Задача:** всё статичное про инфраструктуру клиента — объекты, типы оборудования, единицы оборудования (активы), QR.

**Владеет данными:**
- `Site` — магазин, кафе, цех
- `EquipmentCategory` — каталог типов («холодильная витрина», «компрессор»)
- `Asset` — конкретная единица: «Витрина №2 в зале», инв. номер, QR-код

**Кто пользуется:**

| Роль | Действие |
|------|----------|
| admin | CRUD объектов, категорий, активов; печать QR |
| admin | Подтверждает `draft Asset` от Паспортиста |
| engineer | Читает карточку актива, сканирует QR |
| dispatcher | Фильтрует заявки по объекту (читает Site) |
| ai-gateway (Паспортист) | `createDraftAsset` по фото шильдика |

**Не делает:**
- Не ведёт заявки и журнал (dashboard-service)
- Не хранит PDF-паспорта (document-service хранит файлы; catalog только знает `assetId`)
- Не планирует ТО (это dashboard-service, тип `preventive`)
...
**События:** публикует `asset.draft.created`, `asset.activated` когда админ подтвердил карточку.
...
2. **Плановая (фаза 2):** admin загрузил ИЭ → Технолог создал draft-пакет → диспетчер подтвердил → через 90 дней scheduler создаёт `preventive` → инженер выполняет как обычную заявку.
...
| Роль | Действие |
|------|----------|
| admin | Загружает паспорт к активу |
...
**Пример:** админ загрузил `passport_vitrina.pdf` к активу «Витрина №2» → файл в S3 → событие `document.attached` → search индексирует → Технолог (фаза 2) создаёт draft-пакет в dashboard-service.
...
| Роль | Действие |
|------|----------|
| admin | Дашборд: сколько просроченных заявок по сети |
| reporter | Конструктор отчёта: объект + период + тип → таблица → Excel |
| admin | Выгрузка журнала за квартал для проверки |
...
| `asset.draft.created` | admin | push: «Подтвердите карточку актива» |
...
sequenceDiagram
  ...
  Admin->>DOC: POST /documents (PDF к активу)
