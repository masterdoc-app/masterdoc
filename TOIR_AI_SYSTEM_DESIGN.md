...
> Скоуп: проектируем **работающую систему** — учёт, заявки, журналы, регламенты, роли и ИИ в ежедневных операциях. Склад ЗИП, кладовщик, подрядчики, массовый AI-онбординг (пакетный ввод парка, оцифровка журналов, импорт Excel) сознательно вынесены за рамки первых релизов — в backlog.
...
flowchart TB
  ...
  Journal --> Analyst
  Request --> Analyst
  Analyst -->|дайджест и отчёты:<br/>просрочки, повторы| User
```
...
1. **Категория уже назвала паттерны, их можно не изобретать:** текст/фото → заявка; RAG-чат по мануалам у станка (все); мануал → процедура/чек-лист (MaintainX, UpKeep); фото шильдика → актив (Limble, MaintainX). Складские и голосовые сценарии конкурентов фиксируем как backlog, но не включаем в начальный скоуп.
...
| **Аналитик** | Недельный дайджест руководителю и произвольные отчёты для роли reporter: просрочки, повторные отказы, активы-«пожиратели» времени (лайт-версия Fiix Foresight / ТОиР Аналитик) | Агрегаты учёта: заявки, простои, повторы; пороговые правила (повторный отказ, рост просрочек); скоуп объектов пользователя | Только факты из данных с цифрами и ссылками на заявки; без советов «купите новое»; reporter получает read-only отчёт, не изменение данных | queryAggregates, buildReport (read-only) | Тяжёлая LLM (по запросу / 1 вызов в неделю для дайджеста) | Фаза 3 |
...
Репортёр работает только в read-only режиме. Экраны: Конструктор отчёта → фильтры (объекты, период, тип заявки, статус, актив, инженер) → таблица/графики → экспорт PDF/Excel → сохранённые шаблоны отчётов. В фазе 3 агент Аналитик помогает формировать отчёты по текстовому запросу, например: «покажи повторные отказы по холодильным витринам за квартал» или «сформируй отчёт по просроченным заявкам по магазинам».
...
flowchart TB
  ...
  A_AN -- "недельный дайджест:<br/>повторные отказы, просрочки" --> ADMIN
  REPORTER -- "запрос отчёта" --> A_AN
  A_AN -- "таблица / графики / экспорт" --> REPORTER
...
4. **Отчётный / управленческий** (учёт → Аналитик → руководитель/reporter): руководитель смотрит дашборд, подтверждает регламенты, читает дайджест, выгружает журналы к проверке; reporter формирует любые read-only отчёты в рамках доступа.
...
Каждый агент в сценарии видит только своё: Приёмщик — активы объекта и открытые заявки, Наставник — документы конкретной витрины, Писарь — текущую заявку, Аналитик — агрегаты без содержимого документов.
...
| **3** | Дайджест-аналитика руководителю; конструктор отчётов для reporter | Аналитик |
| **Backlog** | Склад ЗИП и кладовщик; заявки на запчасти; массовый AI-онбординг: пакетные фото шильдиков, оцифровка бумажных журналов, импорт Excel промптами; голосовой ввод и STT; внешний инженер/contractor не в штате | Возвращаем по сигналам custdev |
